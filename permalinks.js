/* permalinks.js — the little chain handle that hangs off each feature.
   ...
   The natal chart had one; now the selected-work grid, the userscript rack, the
   tarot spread and the blog viewer carry the same handle, so any single feature
   can be handed to someone as a url instead of "scroll down a bit". The link
   shelf is left out: its icons are all links already, and a handle among them
   would read as a ninth destination.
